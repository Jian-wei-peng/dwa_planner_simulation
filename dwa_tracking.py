...
class DWA():
    ...
    def _evaluate_trajectory(self, candidate_trajectory, robot_state, goal, obstacles):
        ...
        for trajectory in candidate_trajectory:
            ...
            heading_angle, goal_dist = self._heading_cost(trajectory, goal)
            heading_cost.append(heading_angle)
            distance_cost.append(goal_dist)

            velocity_cost.append(self._velocity_cost(trajectory))

        # 如果所有候选轨迹都会发生碰撞, 则返回当前状态作为最优轨迹, 让机器人原地旋转脱困
        if len(obstacle_cost) == 0:
            # 机器人原地旋转脱困
        ...
        for i in range( len(obstacle_cost) ):
            ...
            total_cost_i = ( self.weight_heading  * heading_cost[i]  +
                             self.weight_distance * distance_cost[i] +
    ...
    def _heading_cost(self, trajectory, goal):
        '''
        评价在当前采样速度下产生的轨迹终点位置方向与目标点连线夹角的误差, 轨迹末端位置到目标点位置的距离
        '''
        # 轨迹末端点到目标点的位置偏差
        ...
        heading_angle = abs((angle_h2r - trajectory[-1][2] + math.pi) % (2 * math.pi) - math.pi)

        return heading_angle, goal_dist
    ...
    def _trajectory_predict(self, robot_state, v_sample, w_sample):
        ...
        while time <= self.predict_time:
            predict_state = self._diff_kinematic_model(predict_state, [v_sample, w_sample])
    ...
    def _diff_kinematic_model(self, state, cmd):
        '''
        差速移动机器人运动学模型, 用于推算预测时域内的轨迹
        '''
        state[0] += cmd[0] * math.cos(state[2]) * self.dt
        state[1] += cmd[0] * math.sin(state[2]) * self.dt
        state[2] += cmd[1] * self.dt
...
def get_move_goal(sim_step, dt):
    '''
    动态目标点
    '''
    goal_x = 3 
    goal_y = 0 + sim_step * dt * 0.7
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 动态目标点
        goal = get_move_goal(sim_step, dwa.dt)
